...
let crypto = null
let decryptor = null
async function initializeDecryptor(videoCodec, aesMode, key, iv) {
    if (!crypto)
        crypto = await Module()
    ...
    decryptor = new crypto.Decryptor(videoCodec === 'AV1' ? crypto.Codec.AV1 : crypto.Codec.AVC, aesMode === 'CTR' ? crypto.Mode.CTR : crypto.Mode.CBC, key, 1024 * 1024)
    if (aesMode === 'CBC')
...
onmessage = (event) => {
    ...
    if (event.data.operation === 'init') {
        ...
        initializeDecryptor(event.data.videoCodec, event.data.aesMode, event.data.key, event.data.iv).then(() => {
