...
let crypto = null
let encryptor = null
async function initializeEncryptor(aesMode, key, iv) {
    if (!crypto)
        crypto = await Module()
    ...
    encryptor = new crypto.Encryptor(crypto.Codec.AVC, aesMode === 'CTR' ? crypto.Mode.CTR : crypto.Mode.CBC, key, 1024 * 1024)
    if (aesMode === 'CBC')
...
onmessage = (event) => {
    ...
    if (event.data.operation === 'init') {
        ...
        initializeEncryptor(event.data.aesMode, event.data.key, event.data.iv).then(() => {
